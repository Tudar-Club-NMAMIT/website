...
const Menu = (NavLists: {
  ...
  return (
    <div className="flex justify-center items-center">
      {/* Hamberger*/}
      <div
        className="md:hidden flex w-8 aspect-square float-right m-0 hamburger p-1 "
        id="hamburger"
        onClick={() => {
      ...
      >
        <div
          className={`${
            !menuToggleFlag ? "bg-black" : "bg-white"
          } bar-hamburger`}
          style={
            !menuToggleFlag
              ? { animation: "cross-1 0.2s ease-in forwards" }
              : { animation: "none" }
          }
        ></div>
        <div
          className={`${
            !menuToggleFlag ? "hidden bg-black" : "block bg-white"
          } bar-hamburger`}
        ></div>
        <div
          className={`${
          ...
          style={
            !menuToggleFlag
              ? { animation: "cross-2 0.2s ease-in forwards" }
              : { animation: "none" }
          }
        ></div>
      </div>
      ...
      <div
        className={`${
          menuToggleFlag ? "hidden" : ""
        } container-menu w-full absolute navbar-bg-menu top-0 left-0`}
        style={
          menuToggleFlag
            ? { animation: "none" }
            : { animation: "menu-click-animation 0.2s ease-in" }
        }
        id="menu-container"
      >
